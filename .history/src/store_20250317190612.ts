...
interface StoreState {
  ...
  getNextVolumeDiscount: () => { itemsNeeded: number; discountPercent: number };
}
    ...
    (set, get) => ({
      ...
      getCartTotal: () => {
        ...
        if (unbrokenBundle) {
            ...
            volumeDiscount = nonBundleSubtotal * volumeDiscountRate;
        }
      ...
      getNextVolumeDiscount: () => {
        const { cart } = get();
        const totalItems = cart.reduce((sum, item) => sum + item.quantity, 0);
        
        if (totalItems < 2) {
          return { itemsNeeded: 2 - totalItems, discountPercent: 5 };
        } else if (totalItems < 3) {
          return { itemsNeeded: 3 - totalItems, discountPercent: 10 };
        } else if (totalItems < 5) {
          return { itemsNeeded: 5 - totalItems, discountPercent: 20 };
        }
        return { itemsNeeded: 0, discountPercent: 20 };
      },
    }),
    {
      name: 'game-store',
