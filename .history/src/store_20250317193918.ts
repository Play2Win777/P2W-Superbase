...
interface StoreState {
  ...
  fetchGames: () => void;
  showToast: (message: string, action: string, onAction: () => void, volumeDiscountInfo: string) => void;
  hideToast: () => void;
  getCartTotal: () => {
    subtotal: number;
...
export const useStore = create<StoreState>()(
  persist(
    (set, get) => ({
      ...
      showToast: (message, action, onAction, volumeDiscountInfo) => {
        set({
          toast: {
            message,
            action,
      ...
      getVolumeDiscountInfo: () => {
        const totalItems = get().cart.reduce((total, item) => total + item.quantity, 0);
        
        if (totalItems >= 5) return "You've reached the maximum volume discount!";
        
        const nextTier = 
          totalItems < 2 ? 2 - totalItems :
          totalItems < 3 ? 3 - totalItems :
          totalItems < 5 ? 5 - totalItems : 0;
        ...
        return `Add ${nextTier} more game${nextTier > 1 ? 's' : ''} for ${discount} off`;
      },

      addToCart: (game) => {
        const { cart, showToast, getVolumeDiscountInfo, navigate } = get();
        const existingItem = cart.find((item) => item.id === game.id);
        const pointsToAdd = Math.floor((game.Price_to_Sell_For || 0) * 10);
        const newPoints = get().loyaltyPoints + pointsToAdd;

        let newTier = get().loyaltyTier;
