    ...
    persist(
        (set, get) => ({
            ...
            getCartTotal: () => {
                ...
                let flashSaleSubtotal = 0
                let nonFlashSaleSubtotal = 0

                //If flash sale active
                let flashSaleActive = flashSaleItems.length >= 3

                if(flashSaleActive){
                    //Calculate the flash sale subtotal
                    flashSaleItems.forEach(item => {
                        flashSaleSubtotal += item.Price_to_Sell_For * item.quantity
                    })
                    flashSaleDiscount = flashSaleSubtotal * .25
                }

                nonFlashSaleSubtotal = subtotal - flashSaleSubtotal
                ...
                volumeDiscount = nonFlashSaleSubtotal * volumeDiscountRate
