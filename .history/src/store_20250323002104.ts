  ...
  persist(
    (set, get) => ({
      ...
      getCartTotal: () => {
        ...
        const introSaleDiscount = introSaleSubtotal * introSaleDiscountRate;
        ...
        // Calculate Flash Sale discount (only for non-Intro Sale eligible items)
        const flashSaleDiscountItems = flashSaleItems.filter(item => !isIntroSaleEligible(item));
        const flashSaleDiscountSubtotal = flashSaleDiscountItems.reduce((sum, item) => sum + (item.Price_to_Sell_For * item.quantity), 0);
        const flashSaleDiscount = flashSaleActive ? flashSaleDiscountSubtotal * 0.25 : 0;
