...
export const getResponsiveImage = (id: string, size: 'small' | 'medium' | 'large' = 'medium') => {
    // For mobile devices
    if (size === 'small') {
      return `https://eeikeheizmdybnufjqji.supabase.co/storage/v1/object/public/covers/${id}_small.webp`;
    }
    
    // For laptops/desktops (default)
    if (size === 'medium') {
      return `https://eeikeheizmdybnufjqji.supabase.co/storage/v1/object/public/covers/${id}.webp`;
    }
    
    // For large screens/TVs
    if (size === 'large') {
      return `https://eeikeheizmdybnufjqji.supabase.co/storage/v1/object/public/covers/${id}_large.webp`;
    }
    
    // Fallback
    return `https://eeikeheizmdybnufjqji.supabase.co/storage/v1/object/public/covers/${id}_fback.jpg`;
  };
