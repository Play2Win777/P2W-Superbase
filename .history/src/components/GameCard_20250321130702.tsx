          ...
          <div className={`absolute inset-0 bg-gradient-to-t from-black/20 via-black/0 to-transparent dark:from-black/20 dark:via-black/0 ${showVideo ? 'fade-out' : ''}`}>
            <div className="absolute bottom-0 left-0 right-0 p-4 text-white">
              <div className="flex justify-between items-center mb-2">
                <div className="flex gap-2 items-center m1-2"> {/* Moved 2 points from the left edge */}
                  <span className="px-4.5 py-0.25 bg-blue-500/80 rounded text-sm">
                    {`${game.Genre}${game.Sub_Genre ? ` - ${game.Sub_Genre}` : ''}`}
                  </span>
                </div>
              </div>
              <div className="flex items-center gap-0 m1-2"> {/* Moved 2 points from the left edge */}
