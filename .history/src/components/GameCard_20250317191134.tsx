...
export const GameCard: React.FC<GameCardProps> = ({ game }) => {
  ...
  const { getNextVolumeDiscount } = useStore(); // Access getNextVolumeDiscount from the store
  ...
  const handleClick = () => {
    ...
    // Calculate the next volume discount
    const { itemsNeeded, discountPercent } = getNextVolumeDiscount();
    if (itemsNeeded > 0) {
      setToastMessage(`Add ${itemsNeeded} more for ${discountPercent}% off!`);
    } else {
