...
export const GameCard: React.FC<GameCardProps> = ({ game }) => {
  ...
  const handlePlatformIconClick = (platform: string) => {
    navigate('/', { state: { platform } }); // Navigate to the main page with platform state
  };
            ...
            <div className="absolute bottom-0 left-0 right-0 p-4 text-white">
              <div className="flex justify-between items-center mb-2">
                <div className="flex gap-2 items-center">
                  <div className="gamecard-icon-container">
                    <img
                      src={getPlatformIcon(game.Platform)}
                      alt={game.Platform}
                      className="h-6 w-auto rounded cursor-pointer hover:opacity-80 transition-opacity"
                      onClick={(e) => {
                        e.stopPropagation();
                        handlePlatformIconClick(game.Platform);
                      }}
                      style={{
                        maxWidth: '100%',
                        maxHeight: '100%',
