...
export const GameCard: React.FC<GameCardProps> = ({ game }) => {
  ...
  const { toastMessage: storeToastMessage, clearToastMessage } = useStore(); // Access toastMessage and clearToastMessage from the store
  ...
  const handleClick = () => {
    ...
    // Show the Toast message from the store
    setShowToast(true);
    setTimeout(() => {
      setShowToast(false);
      clearToastMessage(); // Clear the toast message after 3 seconds
    }, 10000);
  };
