...
export const GameCard: React.FC<GameCardProps> = ({ game }) => {
  ...
  const handleClick = () => {
    ...
    // Calculate the number of items needed for the next volume discount
    const totalItems = useStore.getState().cart.reduce((total, item) => total + item.quantity, 0) + 1;
    let nextDiscount = '';
    if (totalItems < 2) {
      nextDiscount = 'Add 1 more game for a 5% discount!';
    } else if (totalItems < 3) {
      nextDiscount = 'Add 1 more game for a 10% discount!';
    } else if (totalItems < 5) {
      nextDiscount = 'Add 2 more games for a 20% discount!';
    } else {
      nextDiscount = 'You have reached the maximum discount!';
    }

    setToastMessage(`${game.Game_Title} added to cart! ${nextDiscount}`);
    setShowToast(true);
