    ...
    persist(
        (set, get) => ({
            ...
            getCartTotal: () => {
                ...
                const flashSaleActive = flashSaleItems.length >= 3;
                let nonFlashSubtotal = subtotal; // Initialize with the full subtotal

                if (flashSaleActive) {
                    flashSaleItems.forEach(item => {
                        flashSaleDiscount += item.Price_to_Sell_For * 0.25;
                    });
                    nonFlashSubtotal = subtotal - flashSaleDiscount; // Adjust after applying flash sale
                }
                ...
                if (!flashSaleActive) {
                    volumeDiscount = subtotal * volumeDiscountRate; //Volume on everything
                } else {
                    volumeDiscount = (subtotal - flashSaleDiscount) * volumeDiscountRate; //Volume excluding flash
                }
                ...
                return {
                    ...
                    bundleDiscount,
                    flashSaleActive,
                    flashSaleEligibleCount: flashSaleItems.length,
                    total
                };
            }
        }),
