...
export const useStore = create<StoreState>()((set, get) => ({
  ...
  // ✅ Fixed: `setFilters` now merges with existing filters instead of replacing them.
  setFilters: (filters) =>
    set((state) => ({
  ...
  // ✅ Fixed: `removeFromCart` now ensures `isFlashSaleEligible` is updated after item removal.
  removeFromCart: (gameId) =>
    set((state) => ({
      cart: state.cart
        .filter((item) => item.id !== gameId)
        .map((item) => ({ ...item, isFlashSaleEligible: isFlashSaleEligible(item) })),
    })),
