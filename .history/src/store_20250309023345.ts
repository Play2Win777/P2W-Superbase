  ...
  persist(
    (set, get) => ({
      ...
      getCartTotal: () => {
        ...
        // Separate items into mutually exclusive groups
        ...
        const regularItems = nonBundleItems.filter(item => !item.isFlashSaleEligible);
      
        const bundleSubtotal = bundleItems.reduce((sum, item) => 
          sum + (item.Price_to_Sell_For * item.quantity), 0);
        ...
        // Calculate volume discount based on REGULAR items only
        const regularItemsCount = regularItems.reduce((sum, item) => 
          sum + item.quantity, 0);
        let volumeDiscountRate = 0;
        if (regularItemsCount >= 5) volumeDiscountRate = 0.2;
        else if (regularItemsCount >= 3) volumeDiscountRate = 0.1;
        else if (regularItemsCount >= 2) volumeDiscountRate = 0.05;
        const volumeDiscount = regularSubtotal * volumeDiscountRate;
      
        const subtotal = bundleSubtotal + flashSaleSubtotal + regularSubtotal;
