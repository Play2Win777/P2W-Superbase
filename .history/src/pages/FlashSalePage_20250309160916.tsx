 ...
 export const FlashSalePage: React.FC = () => {
  const { games, fetchGames, addToCart } = useStore(); // Add addToCart from store
  const [isLoading, setIsLoading] = useState(true);
  const [clickedGames, setClickedGames] = useState<Set<string>>(new Set()); // Track clicked games
  ...
  // Filter games to only include flash sale eligible games
  const flashSaleGames = games.filter(game => isFlashSaleEligible(game));
  ...
  No flash sale games available at this time.
  </div>
  )}
  </>
  )}
