...
interface StoreState {
  ...
  showFilters: boolean; // Add showFilters state
  setGames: (games: Game[]) => void;
  setFilteredGames: (games: Game[]) => void;
  setCurrentPage: (page: number) => void;
  setSearchQuery: (query: string) => void;
  setFilters: (filters: Partial<FilterState>) => void;
  setShowFilters: (show: boolean) => void; // Add setShowFilters
...
export const useStore = create<StoreState>()(
  persist(
    (set, get) => ({
      ...
      loyaltyPoints: 0,
      loyaltyTier: 'Bronze',
      showFilters: false, // Initialize showFilters
      ...
      setShowFilters: (show) => set({ showFilters: show }), // Add setShowFilters
