...
import { getGameCoverUrl, getGameCoverFallbackUrl } from '../utils/imageUtils'; // Import new utilities
import placeholder from '/src/assets/icons/placeholderlogo_small.webp';
...
export const GameCard: React.FC<GameCardProps> = ({ game }) => {
  ...
  const isIntroEligible = isIntroSaleEligible(game); // Check if the game is eligible for Intro Sale
    ...
    >
      {isEligible && (
        <Link to={`/flash-sale?platform=${game.Platform}`} className="absolute top-2 left-2 z-20">
          <span className="inline-flex items-center bg-gradient-to-r from-red-600 to-orange-500 text-white px-2 py-1 rounded-md text-xs font-semibold animate-pulse">
      ...
      {isIntroEligible && (
        <Link to={`/intro-sale?platform=${game.Platform}`} className="absolute top-10 left-2 z-20">
          <span className="inline-flex items-center bg-gradient-to-r from-blue-500 to-blue-700 text-white px-2 py-1 rounded-md text-xs font-semibold animate-pulse">
            <Zap size={12} className="mr-1" />
            Intro Sale
          </span>
        </Link>
